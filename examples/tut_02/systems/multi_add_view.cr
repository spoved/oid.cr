...
class MultiAddViewSystem < Entitas::MultiReactiveSystem
  ...
  private property top_view_container : Oid::Transform = Oid::GameObject.new("Views").transform
  ...
  def initialize(contexts : Contexts)
    ...
    contexts.all_contexts.each do |ctx|
      ...
      ctx_view_container = Oid::GameObject.new("#{ctx_name} Views").transform
  ...
  def execute(entities : Array(Entitas::IEntity))
    entities.each do |e|
      ...
      go = Oid::GameObject.new("#{ctx_name} View")
      e.add_view(game_object: go)
